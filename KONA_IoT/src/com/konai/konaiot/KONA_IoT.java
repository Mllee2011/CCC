...
public class KONA_IoT extends Applet implements AppletEvent {
	...
	private void IoT_generateKeyPair(APDU apdu,byte[] apduBuffer) {
	    ...
	    byte keyUsage = apduBuffer[ISO7816.OFFSET_P2];   //
	    short lc      = apdu.setIncomingAndReceive();
	    
	    switch (keyType) {
	    	case (byte)0x01:
	        	if ((keyUsage==0x00)&&(lc != 0x07)){
	        		ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);			        	
	        	}
	    		byte keyType_ID  = (byte)(apduBuffer[Constants.OFF_APDU_OBJECT_ID]&0xFF);
			    ...
			    switch (keyType_ID) {
			        case (byte)0x01:
			        	if ((keyUsage==0x01)&&(lc != 0x27)){
			        		ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);//the length check is CSR HASH length 
			        	}	        
